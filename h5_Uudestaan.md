...
---
  ...
  a) tehtävän aloitus 13.02.2024 klo 16.30 UTC+2 ja valmista oli noin klo 19.20 UTC+2
  ...
  1. Aloitin lataamalla Debianin Iso-tiedoston [Debianin sivulta], [debian-live-12.1.0-amd64-xfce.iso](https://cdimage.debian.org/debian-cd/current-live/amd64/iso-hybrid/debian-live-12.5.0-amd64-xfce.iso). Versio oli päivittynyt aiemmin asennetusta versiosta 12.1 versioon 12.5.
...
6. Verkkosivulle tarvitaan sisältö, jonka loin tavallisena käyttäjänä, eli en pääkäyttäjän oikeuksin. Ensin kansio `mkdir -p /home/kadi/publicsites/esim.example.com` ja sitten tiedosto `micro index.html` jonne tallennan sisällön joka näytetään esim.example.com sivulla.
...
Tässä tehtävässä minun tuli tutkia vuokraamani domani-nimen tietoja `host`ja `Dig`komennoilla, sekä analysoida tulokset. Lisäksi tuloksia tuli vertailla domain-nimeni vuokraajan - minun tapauksessa Namecheap - weppiliittymässä näkyviin asetuksiin. Tehtävän aloitus 22.02.2024 klo 18.05 UTC+2 ja valmista oli 22.02.2024 klo 19.20 UTC+2, joten aikaa kului 1h15min. Käytetyt lähteet ovat merkittynä tekstiviitteisiin, sekä lähdeluetteloon. 
...
3. Näistä voimme päätellä seuraavia seikkoja
   ...
   - Seuraavien rivien kohdalla, en löytänyt yksiselitteistä vastauta . Kuitenkin tekstistä voin päätellä että nämä liittyvät sähköpostin uudelleenohjaukseen. Lisäksi Namecheapin asetussivulla Mail settings kohdassa mainitaan sähköpostin uudelleenohjauksesta:
...
4. Komennot `vagrant init debian/bookworm64` ja `vagrant up` asensivat virtuaalikoneen vagrantilla
