...
- Jatkoin Guest additions asennuksella
  > Devises valikosta `Insert Guest Additions CDRom...`
  ...
  Käytän tehtävän teossa ajanottoappia, sillä työskentely oli hyvin pirstaleista.
 ...
 ## b) tiivistelmä koko työstä
 - c) Ei kolmea sekaseiskaa - lopputulos
...
Asenna apache
> - `sudo apt-get -y install apache2` asensin
...
> - Vaihdoin SSH-palvelimen porttiin muokkaamalla komennolla `sudo micro /etc/ssh/sshd_config` kyseistä tiedostoa, jonna vaihdoin portin 22 tilalle 1337, ja otin määrityksen käyttöön poistamalla # edestä.
...
>  > Palasin yön jälkeen hommiin huomatakseni, että en päässyt enään kirjautumaan. Edellisessä asennuksessa  laitoin täpän kohtaan `log in automatically...`, joka taisi kostautua. Joten aloitimpa uudelleen Debianin asennuksen. Tällä kertaa kellotin tarkasti toimet, jotka tein yo raporttia seuraten. Tässä en siis toistanut raporttia niiltä ,osin kuin homma sujui. Ainoastaan ne kohdat joissa tuli vastoinkäymisiä.
...
> - Siirryin käyttämään erkkiä uuteen terminaali-ikkunaan ja loin erkille oman projektikansion kotihakemistoon `mkdir django`.
...
> - Selaimella osoitteessa **127.0.0.1:8000/** oletussivu kertoi asennuksen onnistuneen.
  ...
  > - ympäristö ylös ja testasin osoitteessa **127.0.0.1:8000/admin/** luotuja tunnuksia, toimi.
...
4. Taulu Donations jossa jokaisella tietueella oma nimi
  > - `./manage.py startapp tietokanta` loi tietokanta app`n
...
> - `sudo a2ensite lahjat.conf` otti conffin käyttöön ja `sudo a2dissite 000-default.conf`poisti oletuksen käytöstö
> - `sudo apt-get -y install libapache2-mod-wsgi-py3` asensi Apache wsgi-moduulin.
> - `/sbin/apache2ctl configtest` testasi syntaksi oikeaksi.
> - Otin määritykset käyttöön `sudo systemctl restart apache2`
> - Testi `curl -s localhost|grep title`antaa herjan 403 Forbidden title ja selain keroo puuttuvista oikeuksista.
> - Tarkisin apachen error login `sudo tail /var/log/apache2/error.log`joka osasi kertoa puuttuvia oikeuksia oli.
>   > ![repo10](https://github.com/syjaka/Linux-Palvelimet-2024/blob/main/images/repo10.png)
> - Tarkistin koko errorin polun `ls -l`komennoilla samalla, ainoastaan site-packages hakemistosta puuttui. Komenolla `sudo chmod u=rwx,g=rx,o=rx /home/kadi/publicwsgi/env/lib/python3.11/site-packages/`annoin ne.
> - Testasin ja sama virhe jatkui lokissa, eli se ei tarjonnut lisätietoa. Tarkistin conf tiedoston ja siellä oli vääriä polkuja. Korjasin ne ja uudelleenkäynnistin apachen. Jolloin sekä selain että `curl -s localhost|grep title`antoivat toivotun lopputuloksen.
> - Siirryin lahjat projektin juurikansioon ja muokkasin sieltä `settings.py` tiedostoa kohdista DDEBUG false ja ALLOVED HOSTS "localhost"
...
> - `./manage.py collectstatic` kopioi staattiset tiedostos määrittelemääni hakemistoon.
> - Testasin kirjautumalla aiemmin luodulla kadi superuserina, ja nyt luotu tietokanta näkyy sivulla.
