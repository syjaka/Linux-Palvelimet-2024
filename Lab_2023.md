...
>  > Palasin yön jälkeen hommiin huomatakseni, että en päässyt enään kirjautumaan. Edellisessä asennuksessa  laitoin täpän kohtaan `log in automatically...`, joka taisi kostautua. Joten aloitimpa uudelleen Debianin asennuksen. Tällä kertaa kellotin tarkasti toimet, jotka tein yo raporttia seuraten. Tässä en siis toistanut raporttia niiltä ,osin kuin homma sujui. Ainoastaan ne kohdat joissa tuli vastoinkäymisiä.
  > 1. Virtuaalikoneen ja debianin asennus: 1:24:50
>  >2. Loput tähän asti: 28:24
...
- Tietokannan muokkaus wepissä (Django admin).
"tietokanta"
- Käyttis erkille ilman ylläpito-oikeuksia.
...
- 
- Taulu Donations jossa jokaisella tietueella oma nimi
  > - `./manage.py startapp tietokanta` loi tietokanta app`n
  > - `micro lahjat/settings.py`avasi settingstiedoston jonne "installed apps kohtaan lisäsin:
  >   
  ...
  > - Kirjauduin erkkinä ja testasin, toimi, mutta lahjoitukset näkyvät donations nimellä. Muokkaan vielä models.py lisäämällä:
  >
  >        from django.db import models
  >         
  >        class Donations(models.Model):
  >            Lahjoittaja  = models.CharField(max_length=100, default='ei määritelty')
  >            Lahjoitus = models.CharField(max_length=100, default='ei määritelty')
  >            hinta = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)
  >
  >       def __str__(self):
                  return f"{self.Lahjoittaja} lahjoitti: {Lahjoitus} arvoltaan: {self.hinta}"
