...
Aloitin alla olevat tehtävät jo ennen varsinaista tehtävänantoa, sillä oma aikatauluni sallii parhaiten tämän tiistai-illan työskentelyn. Tehtävänanto tulee näkyville vasta myöhemmin, joten on hyvin mahdollista, että tehtävänkulkn/tekemisjärjestyksessä on poikkeuksia suhteesa tehtävänantoon. Tulen näitä päivittämään tehtävien perään tarvittaessa.

Tehtäviä tehdessäni toistan tunnilla tehtyjä harjoituksia ja seuraan Tero Karvisen ohjeita otsikolla First Steps on a New Virtual Private Server – an Example on DigitalOcean and Ubuntu 16.04 LTS (Karvinen 2017)

---

  ## a) Oman pilvipalvelimen vuoraus ja asennus - 2024.02.06 klo 14.30 - 15.05 UCT +3
...
- Seuraavaksi avautui tunnilta tuttu sivu jossa valitaan luotavalle virtuaaliserverille määritykset:
  ...
  (tämän osio tehty luennon muistiinpanojen pohjalta)
---
  ## b) SSH-yhteys virtuaalipalvelimeen ja palomuurin asennus
...
Aloitin tehtävän asentamalla apachen. Suritin asennuksen, kuten raportissani [h3_HelloWebServer](https://github.com/syjaka/Linux-Palvelimet-2024/blob/main/h3_HelloWebServer.md) tehtävässä 4 tehtiin. Sen vuoksi selostan tässä lyhyemmin asennuksen. 
  1. Avaan yhteyden vuokrattuun koneeseen oman virtuaalikoneeni terminaalista komennolla 'ssh syrja@104.248.205.0'
  2. Haen ja asennan apchen: 'sudo apt-get update', 'sudo apt-get -y install apache2' ja 
     
(tämä osio tehty luennon muistiinpanojen ja Tero Karvisen -First Steps on a New Virtual Private Server – an Example on DigitalOcean and Ubuntu 16.04 LTS, 2017 pohjalta)
---
